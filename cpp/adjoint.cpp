...
//            std::transform(MatArray.begin(),MatArray.end(),MatArray.begin(), [float Cmet, float Cpla](double ve)
//                    -> std::pow((1-ve),p_)*Cpla_ + std::pow(ve,p_)*Cmet_ ));

            std::transform(MatArray.begin(), MatArray.end(), MatArray.begin(),
                    [Cmet_c = Cmet_, Cpla_c = Cpla_, p_c = p_](double ve) ->
                    double {return std::pow((1-ve), p_c)*Cpla_c + std::pow(ve,p_c)*Cmet_c;});

            std::transform(MatDerArray.begin(), MatDerArray.end(), MatDerArray.begin(),
                    [Cmet_c = Cmet_, Cpla_c = Cpla_,p_c = p_](double ve) ->
