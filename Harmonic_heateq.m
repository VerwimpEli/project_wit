function [f0val, df0dv, fval, dfdv] =  ...
    Harmonic_heateq(v, M, VW, VH, Q, Cmet, Cpla, BC0, BC1, BC2, BC3)
                                                
    % Solve heat equation and calculate gradient
    v = reshape(v, [VW, VH]);
    [Sol,K] = Harmonic_FVM(VW,VH, v, Q, Cmet, Cpla, BC0, BC1, BC2, BC3);
    L = (K')\-scale(ones(VW*VH,1));
    
    f0val = sum(scale(Sol), 'all') ;   
    df0dv = Harmonic_Adjoint_Gradient(VW,VH,v,L,Sol)';
